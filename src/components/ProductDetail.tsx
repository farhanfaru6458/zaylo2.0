...
export default function ProductDetailsClient({ product }: Props) {
  ...
  useEffect(() => {
    async function loadRelated() {
      ...
      const items = await ProductServices.getProductsByCategory(product.category);
  ...
  if (!product || !product.images) return <ProductDetailsSkeleton />;
  ...
  return (
    <div className="container py-5 product-details-page">
      ...
      <div className="row g-5">
        ...
        <div className="col-md-6">
          <span
            className="category-label shadow-lg rounded-4 bg-white p-1"
            style={{
              background:
                "linear-gradient(90deg, rgb(194 183 253 / 77%) 0%, rgb(218 203 255 / 64%) 60%, rgb(242 229 251 / 75%) 100%)",
            }}
          >
            {product.category}
          </span>
          ...
          <div className="price-area bg-white shadow-lg p-3 rounded-4 d-flex align-items-center gap-3">
            ...
            <span className="discount-badge bg-success text-white p-1 rounded-4 shadow-sm">
              10% OFF
            </span>
          </div>
          ...
          <div className="actions mt-5 d-flex align-items-center gap-3">
            <Link
              href={"/cart"}
              className={`${styles.buyNow} btn btn-dark w-25 p-1 rounded-5 shadow-lg`}
              onClick={() => {
            ...
            <button
              ...
              onClick={() => {
                const cart: CartItem[] = JSON.parse(
                  localStorage.getItem("full-cart") || "[]"
                );
      ...
      <div className="description-box shadow-lg mt-5 p-4 rounded-5">
        ...
        <p className="description-text mt-4">{product.description}</p>
      </div>
